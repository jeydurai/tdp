...
class GeneralDashboard(val user: DBObject, val client: MongoClient) extends Runnable{
  def run {
    val bookingColl: MongoCollection = MongoConnection().getMongoCollection(client, "booking_dump2")    
    val gdColl: MongoCollection = MongoConnection().getMongoCollection(client, "general_dashboard")    
    gdColl.drop
    ...
    for (periods <- periodsBuffer /*if periods("year").toInt >= 2015*/) {
      val params = (user("username").toString, periods("year"), periods("quarter"), periods("month"), periods("week"), bookingColl)
      val constraints1: String = user("username").toString + "|" + periods("year")
      val constraints2: String = user("username").toString + "|" + periods("year") + "|" + periods("quarter")
      val constraints3: String = user("username").toString + "|" + periods("year") + "|" + periods("quarter") + "|" + periods("month")
      val constraints4: String = user("username").toString + "|" + periods("year") + "|" + periods("quarter") + "|" + periods("month") + "|" + periods("week")
      val constraintsSet = dataProcessConstraints.toSet
      println(user("username") + " Year: " + periods("year") + " Quarter: " + periods("quarter") + " Month: " + periods("month") + 
          " Week: " + periods("week")
      )
      if (!constraintsSet.contains(constraints1)) {
        //println("till date initiated in constraints1!")
        writeGeneralDashboard(gdColl, processData((user("username").toString, Some(periods("year")), None, None, None, bookingColl)))
      }
      if (!constraintsSet.contains(constraints2)) {
        ...
        writeGeneralDashboard(gdColl, processData((user("username").toString, 
              Some(periods("year")), Some(periods("quarter")), None, None, bookingColl)))
      }
      if (!constraintsSet.contains(constraints3)) {
        //println("till date initiated in constraints3!")
        writeGeneralDashboard(gdColl, processData((user("username").toString, 
              Some(periods("year")), Some(periods("quarter")), Some(periods("month")), None, bookingColl)))
      }
      if (!constraintsSet.contains(constraints4)) {
        //println("till date initiated in constraints4!")
        writeGeneralDashboard(gdColl, processData((user("username").toString, 
              Some(periods("year")), Some(periods("quarter")), Some(periods("month")), Some(periods("week")), bookingColl)))
      }
  ...
  private def processData(params: Tuple6[String, Option[String], Option[String], Option[String], Option[String], MongoCollection]): MongoDBObject = {
      ...
      val year = params._2.getOrElse(None); val quarter = params._3.getOrElse(None); val month = params._4.getOrElse(None); val week = params._5.getOrElse(None)
      val coll = params._6

      val mongoPeriods = MongoDBObject("year" -> year, "quarter" -> quarter, "month" -> month, "week" -> month)
  ...
  private def getMongoMatchObj(params: Tuple6[String, Option[String], Option[String], Option[String], Option[String], MongoCollection]): Tuple10[DBObject, DBObject, DBObject, DBObject, DBObject, DBObject, DBObject, DBObject, DBObject, DBObject] = {
    ...
    val common = MongoQueryObjects.matchObjectByPeriods(Some(user), year, quarter, month, week).getOrElse(MongoDBObject())
    val yoy = MongoQueryObjects.matchObjectByPeriods(Some(user), None, quarter, month, week).getOrElse(MongoDBObject())
    val qoq = MongoQueryObjects.matchObjectByPeriods(Some(user), year).getOrElse(MongoDBObject())
    val mom = MongoQueryObjects.matchObjectByPeriods(Some(user), year, quarter).getOrElse(MongoDBObject())
    val wow = MongoQueryObjects.matchObjectByPeriods(Some(user), year, quarter, month).getOrElse(MongoDBObject())
    val cusprevYear = MongoQueryObjects.matchObjectByPeriods(Some(user), Some(prevYear1), quarter, month, week).getOrElse(MongoDBObject())
    val cusMorePrevYear = MongoQueryObjects.matchObjByPeriodsCustomerNotLast3Years(Some(user), Some((prevYear1, prevYear2, prevYear3)), quarter, month, week).getOrElse(MongoDBObject())
    val cus = MongoQueryObjects.matchObjByPeriodsCustomerNot(Some(user), year, quarter, month, week).getOrElse(MongoDBObject())
    val cusSO = MongoQueryObjects.matchObjByPeriodsCustomerDealsNot(Some(user), year, quarter, month, week).getOrElse(MongoDBObject())
    val par = MongoQueryObjects.matchObjByPeriodsPartnerNot(Some(user), year, quarter, month, week).getOrElse(MongoDBObject())
    val mongoObj = (common, yoy, qoq, mom, wow, cusprevYear, cusMorePrevYear, cus, cusSO, par)
    mongoObj
  }
