...
class GeneralDashboardSubSCMS(val user: DBObject, val client: MongoClient) extends Runnable{
  def run {
    ...
    val prodSerList = List("SELECT", "MM", "GEO_NAMED", "GEO_NON_NA")
  ...
  private def getMongoMatchObj(params: Tuple7[String, Option[String], Option[String], Option[String], Option[String],Option[String], MongoCollection]): Tuple10[DBObject, DBObject, DBObject, DBObject, DBObject, DBObject, DBObject, DBObject, DBObject, DBObject] = {
    ...
    val mongoObj = ps match {
      case Some(v) if v equals "GEO_NON_NA" => {
        val common = MongoQueryObjects.matchObjectBySpecialSubSCMS(Some(user), year, quarter, month, week, MongoQueryObjects.multiSubSCMSQryObj(ps)).getOrElse(MongoDBObject())
        val yoy = MongoQueryObjects.matchObjectBySpecialSubSCMS(Some(user), None, quarter, month, week, MongoQueryObjects.multiSubSCMSQryObj(ps)).getOrElse(MongoDBObject())
        val qoq = MongoQueryObjects.matchObjectBySpecialSubSCMS(Some(user), year, None, None, None, MongoQueryObjects.multiSubSCMSQryObj(ps)).getOrElse(MongoDBObject())
        val mom = MongoQueryObjects.matchObjectBySpecialSubSCMS(Some(user), year, quarter, None, None, MongoQueryObjects.multiSubSCMSQryObj(ps)).getOrElse(MongoDBObject())
        val wow = MongoQueryObjects.matchObjectBySpecialSubSCMS(Some(user), year, quarter, month, None, MongoQueryObjects.multiSubSCMSQryObj(ps)).getOrElse(MongoDBObject())
        val cusprevYear = MongoQueryObjects.matchObjectBySpecialSubSCMS(Some(user), Some(prevYear1), quarter, month, week, MongoQueryObjects.multiSubSCMSQryObj(ps)).getOrElse(MongoDBObject())
        val cusMorePrevYear = MongoQueryObjects.matchObjBySpecialSubSCMSCustomerNotLast3Years(Some(user), Some((prevYear1, prevYear2, prevYear3)), quarter, month, week, MongoQueryObjects.multiSubSCMSQryObj(ps)).getOrElse(MongoDBObject())
        val cus = MongoQueryObjects.matchObjBySpecialSubSCMSCustomerNot(Some(user), year, quarter, month, week, MongoQueryObjects.multiSubSCMSQryObj(ps)).getOrElse(MongoDBObject())
        val cusSO = MongoQueryObjects.matchObjBySpecialSubSCMSCustomerDealsNot(Some(user), year, quarter, month, week, MongoQueryObjects.multiSubSCMSQryObj(ps)).getOrElse(MongoDBObject())
        val par = MongoQueryObjects.matchObjBySpecialSubSCMSPartnerNot(Some(user), year, quarter, month, week, MongoQueryObjects.multiSubSCMSQryObj(ps)).getOrElse(MongoDBObject())
        (common, yoy, qoq, mom, wow, cusprevYear, cusMorePrevYear, cus, cusSO, par)
      }
      case v  => {
        val common = MongoQueryObjects.matchObjectBySubSCMS(Some(user), year, quarter, month, week, ps).getOrElse(MongoDBObject())
        val yoy = MongoQueryObjects.matchObjectBySubSCMS(Some(user), None, quarter, month, week, ps).getOrElse(MongoDBObject())
        val qoq = MongoQueryObjects.matchObjectBySubSCMS(Some(user), year, None, None, None, ps).getOrElse(MongoDBObject())
        val mom = MongoQueryObjects.matchObjectBySubSCMS(Some(user), year, quarter, None, None, ps).getOrElse(MongoDBObject())
        val wow = MongoQueryObjects.matchObjectBySubSCMS(Some(user), year, quarter, month, None, ps).getOrElse(MongoDBObject())
        val cusprevYear = MongoQueryObjects.matchObjectBySubSCMS(Some(user), Some(prevYear1), quarter, month, week, ps).getOrElse(MongoDBObject())
        val cusMorePrevYear = MongoQueryObjects.matchObjBySubSCMSCustomerNotLast3Years(Some(user), Some((prevYear1, prevYear2, prevYear3)), quarter, month, week, ps).getOrElse(MongoDBObject())
        val cus = MongoQueryObjects.matchObjBySubSCMSCustomerNot(Some(user), year, quarter, month, week, ps).getOrElse(MongoDBObject())
        val cusSO = MongoQueryObjects.matchObjBySubSCMSCustomerDealsNot(Some(user), year, quarter, month, week, ps).getOrElse(MongoDBObject())
        val par = MongoQueryObjects.matchObjBySubSCMSPartnerNot(Some(user), year, quarter, month, week, ps).getOrElse(MongoDBObject())
        (common, yoy, qoq, mom, wow, cusprevYear, cusMorePrevYear, cus, cusSO, par)
      }
